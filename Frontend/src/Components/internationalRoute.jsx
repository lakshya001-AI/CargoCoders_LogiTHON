...
function InternationalRoute() {
  ...
  const handleSaveResponse = async () => {
    ...
    const internationalRoute = routeResult.route;
    const internationalCost = shippingCost;
    const internationalTime = shippingTime;
    const userEmailAddress =
      localStorage.getItem("userEmailAddress") || "john.doe@example.com";
      const typeOfRoute = "International Route";
  
    try {
      const response = await axios.post("http://localhost:5000/saveInternationalRouteInfo", {
        typeOfRoute,
        internationalRoute,
        internationalCost,
        internationalTime,
