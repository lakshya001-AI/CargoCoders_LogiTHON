...
function DomesticRoute() {
  ...
  const handleSaveResponse = async () => {
    ...
    const domesticRoute = routeDetails.best_route;
    const domesticCost = routeDetails.cost;
    const domesticTime = routeDetails.time;
    const userEmailAddress =
      localStorage.getItem("userEmailAddress") || "john.doe@example.com";
      const typeOfRoute = "Domestic Route";
  
    try {
      const response = await axios.post("http://localhost:5000/saveDomesticRouteInfo", {
        typeOfRoute,
        domesticRoute,
        domesticCost,
        domesticTime,
