...
app.post("/saveDomesticRouteInfo", async (req, res) => {
  try {
    const { typeOfRoute, domesticRoute, domesticCost, domesticTime, userEmailAddress } = req.body;

    // Check if all fields are provided
    if (!typeOfRoute || !domesticRoute || !domesticCost || !domesticTime || !userEmailAddress) {
    ...
    // Add the route details to the user's domesticRoutes array
    const routeObj = { routeType: typeOfRoute, route: domesticRoute, cost: domesticCost, time: domesticTime };
    user.RoutesData.push(routeObj);

    // Save the user with updated routes
    await user.save();

    res.status(200).send({ message: "Route information saved successfully!" });
  } catch (error) {
    console.error("Error:", error);
    res.status(500).send({ message: "An error occurred while saving route information." });
  }
});


app.post("/saveInternationalRouteInfo", async (req, res)=>{
  const {typeOfRoute,internationalRoute,internationalCost,internationalTime,userEmailAddress,} = req.body;

  try {
    // Check if all fields are provided
    if (!typeOfRoute || !internationalRoute || !internationalCost || !internationalTime || !userEmailAddress) {
      return res.status(400).send({ message: "All fields are required." });
    }
    ...
    // Add the route details to the user's domesticRoutes array
    const routeObj = { typeOfRoute,internationalRoute,internationalCost,internationalTime,userEmailAddress };
    user.RoutesData.push(routeObj);
...
app.post("/fetchUserRoutes", async (req, res) => {
  try {
    const { userEmailAddress } = req.body;

    // Validate email
    if (!userEmailAddress) {
      return res.status(400).send({ message: "User email address is required." });
    }
    ...
    // Return the user's saved routes
    res.status(200).send({ routes: user.RoutesData });
  } catch (error) {
    console.error("Error fetching user routes:", error);
    res.status(500).send({ message: "An error occurred while fetching user routes." });
  }
});
